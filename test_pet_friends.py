...
def test_post_api_pets_valid(name='Кот', animal_type='Домашний Кот', age='2', pet_photo='image\cat1.jpg'):
...
def test_delete_api_pets_valid(filter='my_pets'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, first_list = pf.get_list_of_pets(auth_key, filter)
    pet_id = first_list['pets'][0]['id']
    status = pf.delete_pet(auth_key, pet_id)
    _, second_list = pf.get_list_of_pets(auth_key, filter)
...
def test_delete_api_pets_with_invalid_pet_id(filter='my_pets'):
    ...
    status = pf.put_api_pets(auth_key, pet_id)
